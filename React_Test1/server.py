...
@app.route("/api/daily_country_attacks", methods=["GET"])
def get_daily_country_attacks():
    """
    Fetch daily attack counts for each country from Elasticsearch.
    """
    try:
        query = {
            "size": 0,
            "aggs": {
                "countries": {
                    "terms": {
                        "field": "GeoLocation.country_name.keyword",  # Use keyword field for aggregation
                        "size": 1000  # Adjust size based on expected number of countries
                    },
                    "aggs": {
                        "daily": {
                            "date_histogram": {
                ...
                "bool": {
                    "filter": [
                        {"match_all": {}},
                        {
                            "range": {
                                "timestamp": {
                                    "gte": "now-1d/d",  # Fetch data from the last 24 hours
                                    "lte": "now",
                                    "format": "strict_date_optional_time"
                                }
                            }
        ...
        for bucket in buckets:
            country = bucket["key"]
            daily_buckets = bucket.get("daily", {}).get("buckets", [])
            daily_count = sum(interval["doc_count"] for interval in daily_buckets)
            results[country] = daily_count

        return jsonify(results)

    except requests.exceptions.RequestException as e:
        return jsonify({"error": f"Error fetching daily country attacks: {e}"}), 500
